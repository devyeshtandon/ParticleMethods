...
class Node(object):
	...
	def multipoleEx(self, p):
		...
		print p.xy
		print conj(v)
		print p.fieldValue(self.center)
	#	assert(abs(conj(v) + p.fieldValue(self.center))<0.1)
		return conj(v)
...
def NodeMain(particles):
	...
	print [i.aj for i in N]
	CalculateVelocity(particles, N, tree)
#	print [i.velocity for i in particles]
	return N, tree
