...
def vortex_source_field(strength, new_coods, location):
	...
	for i in range(len(distance)):
		if distance[i] != 0:
			...
			strength = 4*y/(1-y**2)
			blob = 0.02
			temp[i] = strength*distance[i]*1.0/2/pi/(distance[i]**2 + blob**2)
		else:
			temp[i] = 0+0*1j
	return temp
...
def doublet_field(strength, new_coods, direction):
	...
	for i in range(len(new_coods)):
		...
		if distance != 0:
			...
			if direction == "t":
				temp[i] = -(temp[i]*(new_coods[i].imag/distance))
			if direction == "r":
				temp[i] = -(temp[i]*new_coods[i].real/distance)
		else:
			temp[i] = 0+0*1j
	return temp
...
def vortex(element, target, *argv):
	...
	if element.ele_type == "vortex":
		field_mag = vortex_source_field(strength, new_coods, location)   
		...
		field_mag = [strength for i in new_coods]
		field_dir = [1+0*1j for i in new_coods]
